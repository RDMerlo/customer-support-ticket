@extends('public.layouts.index')

@section('content')
        @include('public.blocks_template.feedback')
{{--        @include('public.blocks_template.about')--}}
{{--        @include('public.blocks_template.researches')--}}
{{--        @include('public.blocks_template.events_block.events')--}}
{{--        @include('public.blocks_template.past_events')--}}
{{--        @include('public.blocks_template.experts')--}}
{{--        @include('public.blocks_template.knowledge')--}}
{{--        @include('public.blocks_template.practices')--}}
{{--        @include('public.blocks_template.infographics')--}}
{{--        @include('public.blocks_template.feedback')--}}
{{--        @include('public.blocks_template.subscription_and_contacts')--}}
@endsection
